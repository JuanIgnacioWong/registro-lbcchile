<x-admin-layout>
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.clubs.store') }}" class="max-w-3xl space-y-4 rounded-xl bg-white p-6 shadow-sm" x-data="{name:'{{ old('name') }}', slug:'{{ old('slug') }}'}">
        @csrf
        <div>
        ...
        <div>
            ...
            <select name="division_id" class="mt-1 w-full rounded border-slate-300">
                @foreach($divisions as $division)<option value="{{ $division->id }}" @selected(old('division_id') == $division->id)>{{ $division->season->year }} - {{ $division->name }}</option>@endforeach
            </select>
        </div>
        ...
        <label class="flex items-center gap-2"><input type="checkbox" name="is_active" value="1" checked> Activo</label>
